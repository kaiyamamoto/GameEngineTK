...
// 地形データの描画
//--------------------------------------------------------------------------------------
void LandShape::Draw(const DirectX::CommonStates& state, const DirectX::SimpleMath::Matrix & view, const DirectX::SimpleMath::Matrix & proj)
{
...
bool LandShape::IntersectSphere(const Sphere& sphere, Vector3* reject)
{
	...
	for (int i = 0; i < nTri; i++)
	{
		...
		// 三角形と球の当たり判定
		if (CollisionNode::CheckSphere2Triangle(localsphere, tri, &temp_inter))
		{// ヒットした
			hit = true;
			// 衝突点から球の中心へのベクトル
